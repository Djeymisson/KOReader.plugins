...
-- Extra args, when given, are forwarded to fn via pcall instead of being
-- captured by a wrapper closure. This matters here because several callers
-- run on every header/footer repaint, and a fresh closure per call is wasted
-- allocation on an e-ink device's hot paint path.
...
function ReaderHeaderFooter:setFontSize(font_size)
    ...
    self.refresh_region_font_size = math_max(old_font_size, new_font_size)
...
function ReaderHeaderFooter:getWifiState()
    return safe_call(NetworkMgr.isWifiOn, false, NetworkMgr)
end
...
function ReaderHeaderFooter:getPowerSnapshot()
    ...
    return {
        level = safe_call(powerd.getCapacity, nil, powerd),
        charging = safe_call(powerd.isCharging, false, powerd),
    }
end
...
function ReaderHeaderFooter:onReaderReady()
    ...
    if self.ui and self.ui.document then
        self.pages = safe_call(self.ui.document.getPageCount, nil, self.ui.document)
    end
...
function ReaderHeaderFooter:getCurrentPage()
    ...
    if self.ui and self.ui.document then
        return safe_call(self.ui.document.getCurrentPage, 1, self.ui.document)
    end
...
function ReaderHeaderFooter:getChapterTitle(pageno)
    ...
    return safe_call(self.ui.toc.getTocTitleByPage, "", self.ui.toc, pageno) or ""
end
...
function ReaderHeaderFooter:getChapterSubtitle(pageno)
    ...
    if self.ui.toc.getTocSubtitleByPage then
        subtitle = safe_call(self.ui.toc.getTocSubtitleByPage, "", self.ui.toc, pageno) or ""
    end
    ...
    if self.ui.toc.getTocSubTitleByPage then
        return safe_call(self.ui.toc.getTocSubTitleByPage, "", self.ui.toc, pageno) or ""
    end
...
function ReaderHeaderFooter:isChapterStart(pageno)
    ...
    local pages_done = safe_call(self.ui.toc.getChapterPagesDone, nil, self.ui.toc, pageno)
...
function ReaderHeaderFooter:getPagesLeftInBook()
    ...
    if pages and pages > 0 and pageno then
        return math_max(0, pages - pageno)
    end

    if self.ui and self.ui.document then
        return safe_call(self.ui.document.getTotalPagesLeft, 0, self.ui.document, pageno)
    end
...
function ReaderHeaderFooter:getPagesLeftInChapter()
    ...
    if self.ui and self.ui.toc then
        local left = safe_call(self.ui.toc.getChapterPagesLeft, nil, self.ui.toc, pageno)
...
function ReaderHeaderFooter:getPercentageRead()
    ...
    if pages and pages > 0 then
        return math_min(100, math_max(0, (pageno / pages) * 100))
    end
...
function ReaderHeaderFooter:getBatteryText()
    ...
    local level = safe_call(powerd.getCapacity, nil, powerd)

    if not level then
        return ""
    end

    local charging = safe_call(powerd.isCharging, false, powerd)
    local charged = safe_call(powerd.isCharged, false, powerd)
    local icon = safe_call(powerd.getBatterySymbol, "", powerd, charged, charging, level)
...
function ReaderHeaderFooter:getContentHorizontalBounds()
    ...
    if self:usesDocumentMargins() then
        ...
        if self.ui and self.ui.document and self.ui.document.getPageMargins then
            local ok, margins = pcall(self.ui.document.getPageMargins, self.ui.document)

            if ok and type(margins) == "table" then
                local left_margin = margins.left or 0
                local right_margin = margins.right or 0

                left_x = math_max(left_x, offset_x + left_margin)
                right_x = math_min(right_x, screen_w - offset_x - right_margin)
            end
        end
        ...
        if self.visible_area then
            left_x = math_max(left_x, self.visible_area.x + pad)
            right_x = math_min(right_x, self.visible_area.x + self.visible_area.w - pad)
        end
    else
...
function ReaderHeaderFooter:getIndicatorRefreshRegions()
    ...
    local top_region = Geom:new({
        x = math_max(0, left_bound - extra),
        y = math_max(0, top_pad - extra),
        w = math_min(screen_w, region_w),
        h = line_h + extra * 2,
    })

    local bottom_region = Geom:new({
        x = math_max(0, left_bound - extra),
        y = math_max(0, screen_h - line_h - bottom_pad - extra),
        w = math_min(screen_w, region_w),
        h = line_h + extra * 2,
    })
...
-- Measures width via RenderText directly instead of building/freeing a
-- TextWidget. This runs on every header/footer repaint (and, through
-- fitTextToWidth below, potentially several times per repaint), so skipping
-- the widget allocation matters on an e-ink device.
function ReaderHeaderFooter:getTextSize(text)
    local tsize = RenderText:sizeUtf8Text(0, Screen:getWidth(), self.font_face, text, true)
    return { w = tsize.x }
end
...
function ReaderHeaderFooter:fitTextToWidth(text, max_width)
    ...
    local ellipsis = "..."
    local ellipsis_width = self:getTextSize(ellipsis).w
    -- getSubTextByWidth walks whole UTF-8 characters, unlike trimming the
    -- string one byte at a time, which could otherwise split a multi-byte
    -- character (accented letters, the Wi-Fi glyph, etc.) in two.
    local fitted = RenderText:getSubTextByWidth(
        text, self.font_face, math_max(0, max_width - ellipsis_width), true
    )
...
function ReaderHeaderFooter:paintTo(bb, x, y)
    ...
    if rt_text ~= "" then
        ...
        if not self:hasRightTopIndicatorsEnabled() and lt_text ~= "" then
            max_right_w = math_max(40, math_floor(usable_w * 0.5))
        end
    ...
    if lt_text ~= "" then
        ...
        if rt_size.w > 0 then
            max_left_w = math_max(40, usable_w - rt_size.w - gap)
        end
    ...
    if self:showsReadPercentage() then
        local rb_text = string.format("%d%%", math_floor(self:getPercentageRead()))
    ...
    if self:showsFooterLeft() then
        ...
        if rb_size.w > 0 then
            max_left_w = math_max(40, usable_w - rb_size.w - gap)
        end
