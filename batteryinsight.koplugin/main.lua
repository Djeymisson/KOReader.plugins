...
local DEFAULT_SAMPLE_INTERVAL_MIN = 30
local MAX_SAMPLES = 1200
local GRAPH_HOURS = 24
local DEFAULT_PROCESS_INTERVAL_MIN = 60
local PROCESS_SCAN_LIMIT = 30
...
local function readProcStat(pid)
	...
	local real_pid = line:match("^(%d+)%s+%(") or tostring(pid)
	local name = line:match("%((.*)%)") or tostring(real_pid)
	...
	return {
		pid = tostring(real_pid),
		name = name,
		ticks = (tonumber(fields[12]) or 0) + (tonumber(fields[13]) or 0),
	}
end

local function addCandidatePid(list, seen, pid, limit)
	pid = tostring(pid or ""):match("^(%d+)$")
	if not pid or seen[pid] or #list >= limit then
		return
	end
	seen[pid] = true
	list[#list + 1] = pid
...
local function readPidsFromCommand(command, list, seen, limit)
	...
	for line in p:lines() do
		...
		addCandidatePid(list, seen, pid, limit)
		if #list >= limit then
			break
		end
	end
...
local function processSnapshotCandidates(previous, limit)
	local list, seen = {}, {}
	limit = limit or PROCESS_SCAN_LIMIT

	local self_stat = readProcStat("self")
	if self_stat then
		addCandidatePid(list, seen, self_stat.pid, limit)
	end

	if previous then
		for pid in pairs(previous) do
			addCandidatePid(list, seen, pid, limit)
			if #list >= limit then
				return list
			end
	...
	readPidsFromCommand("ps 2>/dev/null", list, seen, limit)

	if #list < limit then
		local p = io.popen("ls -1 /proc 2>/dev/null")
		if p then
			for line in p:lines() do
				addCandidatePid(list, seen, line, limit)
				if #list >= limit then
					break
				end
			end
...
local function readProcessSnapshot(previous, limit)
	local snapshot = {}
	for _, pid in ipairs(processSnapshotCandidates(previous, limit or PROCESS_SCAN_LIMIT)) do
		local stat = readProcStat(pid)
		if stat then
...
local function getWifiState()
	local candidates = {
		function()
			return Device:isWifiOn()
		end,
		function()
			return Device.wifi_enable
		end,
		function()
			return Device.network_manager and Device.network_manager:isWifiOn()
		end,
	}
...
local function getFrontlightLevel()
	local candidates = {
		function()
			return Device.frontlightIntensity
		end,
		function()
			return Device.screen and Device.screen:getFrontlightLevel()
		end,
		function()
			return PowerD:getFrontlightLevel()
		end,
	}
...
local function sampleDrainRate(first, last)
	...
	if type(first.capacity) ~= "number" or type(last.capacity) ~= "number" then
		return nil
	end
	if type(first.wall_ts) ~= "number" or type(last.wall_ts) ~= "number" then
		return nil
	end
	if last.wall_ts <= first.wall_ts then
		return nil
	end
	if last.charging or last.capacity >= first.capacity then
		return nil
	end
...
local function estimateRemaining(samples)
	...
	if not last or type(last.capacity) ~= "number" or last.charging then
		return nil
	end

	local rate = averageDrain(samples, 6 * 3600) or averageDrain(samples, 24 * 3600) or averageDrain(samples, 72 * 3600)
...
local BatteryInsightState = {
	...
	process_diagnostics_enabled = true,
	process_interval_min = DEFAULT_PROCESS_INTERVAL_MIN,
	scheduled = false,
	cpu_snapshot = nil,
	process_snapshot = nil,
	last_processes = {},
	last_cpu_usage = nil,
	last_process_wall_ts = nil,
	dashboard = nil,
}
...
function BatteryInsightState:init()
	...
	if self.sample_interval_min < DEFAULT_SAMPLE_INTERVAL_MIN then
		self.sample_interval_min = DEFAULT_SAMPLE_INTERVAL_MIN
	end

	self.process_diagnostics_enabled = self.settings:readSetting("process_diagnostics_enabled")
	if self.process_diagnostics_enabled == nil then
		self.process_diagnostics_enabled = true
	end

	self.process_interval_min = self.settings:readSetting("process_interval_min") or DEFAULT_PROCESS_INTERVAL_MIN
	if self.process_interval_min < 30 then
		self.process_interval_min = DEFAULT_PROCESS_INTERVAL_MIN
	end

	self.cpu_snapshot = self.settings:readSetting("cpu_snapshot")
	self.process_snapshot = self.settings:readSetting("process_snapshot")
	self.last_processes = self.settings:readSetting("last_processes") or {}
	self.last_cpu_usage = self.settings:readSetting("last_cpu_usage")
	self.last_process_wall_ts = self.settings:readSetting("last_process_wall_ts")
	self:trimSamples()
end
...
function BatteryInsightState:flush()
	self.settings:reset({
		...
		process_diagnostics_enabled = self.process_diagnostics_enabled,
		process_interval_min = self.process_interval_min,
		cpu_snapshot = self.cpu_snapshot,
		process_snapshot = self.process_snapshot,
		last_processes = self.last_processes,
		last_cpu_usage = self.last_cpu_usage,
		last_process_wall_ts = self.last_process_wall_ts,
	})
	self.settings:flush()
end

function BatteryInsightState:batteryChanged(sample)
	local last = self.samples[#self.samples]
	if not last then
		return true
	end
	return last.capacity ~= sample.capacity or last.charging ~= sample.charging or last.charged ~= sample.charged
end

function BatteryInsightState:captureBattery(reason, force_save)
	...
	if force_save or self:batteryChanged(sample) then
	...
	return self.samples[#self.samples] or sample, false
end

function BatteryInsightState:shouldCollectProcessDiagnostics(force)
	if not self.process_diagnostics_enabled then
		return false
	end
	if force then
		return true
	end
	if not self.last_process_wall_ts then
		return true
	end
	return wallClock() - self.last_process_wall_ts >= math.max(30, self.process_interval_min) * 60
end

function BatteryInsightState:collectProcessDiagnostics(reason, force)
	if not self.process_diagnostics_enabled then
		self.last_processes = {}
		self.last_cpu_usage = nil
		return nil
	end

	if not self:shouldCollectProcessDiagnostics(force) then
		return self.last_processes
	end

	local cpu = readCpuStat()
	local proc = readProcessSnapshot(self.process_snapshot, PROCESS_SCAN_LIMIT)
	...
	self.last_processes = top_processes
	self.last_cpu_usage = cpu_usage
	self.last_process_wall_ts = wallClock()
	self:flush()
...
function BatteryInsightState:capture(reason)
	return self:captureBattery(reason, false)
end

function BatteryInsightState:scheduleNext()
	if self.scheduled or not self.enabled then
		return
	end

	self.scheduled = true
	UIManager:scheduleIn(math.max(DEFAULT_SAMPLE_INTERVAL_MIN, self.sample_interval_min) * 60, function()
		self.scheduled = false
		if self.enabled then
			self:captureBattery("timer", false)

			if self.dashboard then
				self:collectProcessDiagnostics("timer", false)
...
function BatteryInsightState:resetHistory()
	...
	self.last_processes = {}
	self.last_cpu_usage = nil
	self.last_process_wall_ts = nil
	self:captureBattery("reset", true)
	self:flush()
end

function BatteryInsightState:setInterval(minutes)
	self.sample_interval_min = math.max(DEFAULT_SAMPLE_INTERVAL_MIN, minutes)
	self:flush()
	self:scheduleNext()
end

function BatteryInsightState:toggleProcessDiagnostics()
	self.process_diagnostics_enabled = not self.process_diagnostics_enabled
	if not self.process_diagnostics_enabled then
		self.last_processes = {}
		self.last_cpu_usage = nil
		self.last_process_wall_ts = nil
...
function DashboardWidget:refreshData()
	...
	self.data = {
		...
		process_diagnostics_enabled = self.state.process_diagnostics_enabled,
		top_processes = self.state.last_processes or {},
		last_process_wall_ts = self.state.last_process_wall_ts,
		last_cpu_usage = self.state.last_cpu_usage,
	}
end
...
function DashboardWidget:drawMainGraph(bb, x, y, w, h)
	...
	if #samples < 2 then
		self:drawText(
			bb,
			_("Collect more samples to draw the battery curve."),
			plot_x + 8,
			plot_y + math.floor(plot_h / 2) - 12,
			self.face_small,
			{ fgcolor = COLOR_DARK }
		)
		return
	...
	self:drawText(
		bb,
		first and formatTime(first.wall_ts) or "",
		plot_x,
		label_y,
		self.face_axis or self.face_tiny or self.face_small,
		{ fgcolor = COLOR_DARK }
	)
	self:drawText(
		bb,
		last and formatTime(last.wall_ts) or "",
		plot_x + plot_w - Screen:scaleBySize(90),
		label_y,
		self.face_axis or self.face_tiny or self.face_small,
		{
			fgcolor = COLOR_DARK,
...
function DashboardWidget:drawProcessList(bb, x, y, w, h)
	local top = self.data.top_processes or {}

	bb:paintBorder(x, y, w, h, 2, COLOR_BLACK, 0)
	self:drawText(bb, _("Potential battery drainers"), x + 14, y + 14, self.face, { bold = true })

	local subtitle = _("Recent CPU activity since the previous process sample")
	if self.data.last_process_wall_ts then
		subtitle = subtitle .. " · " .. formatTime(self.data.last_process_wall_ts)
	end
	self:drawText(bb, subtitle, x + 14, y + 54, self.face_small, { fgcolor = COLOR_DARK })
	...
	if not self.data.process_diagnostics_enabled then
		self:drawText(
			bb,
			_("Process diagnostics are disabled."),
			x + 18,
			header_y + 10,
			self.face_small,
			{ fgcolor = COLOR_DARK }
		...
		self:drawText(
			bb,
			_("Enable them from the Battery Insight menu."),
			x + 18,
			header_y + 42,
			self.face_tiny or self.face_small,
			{ fgcolor = COLOR_DARK }
		)
		return
	end
	...
	self:drawText(
		...
		header_y,
		self.face_tiny or self.face_small,
		{ fgcolor = COLOR_DARK, align = "right", w = Screen:scaleBySize(90) }
	)
	...
	if #top == 0 then
		self:drawText(
			bb,
			_("The process list appears after two process samples."),
			x + 18,
			yy + 10,
			self.face_small,
			{ fgcolor = COLOR_DARK }
		...
		self:drawText(
			bb,
			_("Tap the center to collect another sample."),
			x + 18,
			yy + 42,
			self.face_tiny or self.face_small,
			{ fgcolor = COLOR_DARK }
	...
	for i = 1, shown do
		...
		self:drawText(
			...
			yy,
			self.face_tiny or self.face_small,
			{ align = "right", w = Screen:scaleBySize(90) }
		)
		yy = yy + line_h
	end
	...
	if shown < #top and yy <= y + h - 28 then
		self:drawText(
			bb,
			string.format(_("+%d more"), #top - shown),
			x + 18,
			yy,
			self.face_tiny or self.face_small,
			{ fgcolor = COLOR_DARK }
...
function DashboardWidget:paintTo(bb, x, y)
	...
	self:drawText(
		bb,
		VERSION,
		x + size.w - margin - Screen:scaleBySize(86),
		cursor_y + 4,
		self.face_tiny or self.face_small,
		{
			align = "right",
	...
	self:drawText(
		...
		margin,
		cursor_y + Screen:scaleBySize(40),
		self.face_small,
		{ fgcolor = COLOR_DARK }
	)
	...
	self:drawBatteryBar(
		bb,
		battery_bar_x,
		percent_y + Screen:scaleBySize(12),
		battery_bar_w,
	...
	self:drawText(
		bb,
		last and (_("Last sample: ") .. formatDateTime(last.wall_ts)) or _("No samples"),
		margin,
		footer_y + 10,
		self.face_tiny or self.face_small,
		{ fgcolor = COLOR_DARK }
	...
	self:drawText(
		bb,
		_("Tap left: close · center: refresh · right: reset history"),
		margin,
		footer_y + 38,
		self.face_tiny or self.face_small,
		{ fgcolor = COLOR_DARK }
...
function BatteryInsightState:showDashboard()
	self:captureBattery("view", false)
	self:collectProcessDiagnostics("view", true)
	...
	function container:onRefresh()
		BatteryInsightState:captureBattery("manual", false)
		BatteryInsightState:collectProcessDiagnostics("manual", true)
...
function BatteryInsight:init()
	...
	BatteryInsightState:captureBattery("start", false)
...
function BatteryInsight:addToMainMenu(menu_items)
	menu_items.battery_insight = {
		...
		sub_item_table = {
			...
			{
				text = _("Enable process diagnostics"),
				checked_func = function()
					return BatteryInsightState.process_diagnostics_enabled
				end,
				callback = function(touchmenu_instance)
					BatteryInsightState:toggleProcessDiagnostics()
...
function BatteryInsight:onSuspend()
	BatteryInsightState:captureBattery("suspend", false)
end

function BatteryInsight:onResume()
	BatteryInsightState:captureBattery("resume", false)
	BatteryInsightState:scheduleNext()
end

function BatteryInsight:onCharging()
	BatteryInsightState:captureBattery("charging", false)
end

function BatteryInsight:onNotCharging()
	BatteryInsightState:captureBattery("not_charging", false)
end
