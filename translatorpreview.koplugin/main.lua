...
local BUTTON_FONT_SIZE = math_max(12, UI_FONT_SIZE - 3)
...
local HEADER_SEPARATOR_HEIGHT = math_max(1, Screen:scaleBySize(1))
local HEADER_SEPARATOR_GAP = Screen:scaleBySize(6)
local LANGUAGE_FONT_SIZE = math_max(12, UI_FONT_SIZE - 3)
...
local function estimateHtmlHeight(html, content_width, font_size, max_height)
    ...
    lines = math_max(1, lines)
    local line_height = math_ceil(font_size * 1.30)
    local safety = lines <= 2 and Screen:scaleBySize(8) or Screen:scaleBySize(18)
    local estimated_height = math_ceil(lines * line_height + safety)
    local min_height = math_max(Screen:scaleBySize(56), estimated_height)

    if max_height and max_height > 0 then
        return math_max(1, math_min(max_height, min_height))
    end
...
function PreviewButton:init()
    local bordersize = 0
    local padding_h = math_max(2, math_floor(Size.padding.button * 0.75))
    local padding_v = math_max(1, math_floor(Size.padding.button * 0.55))
...
function TranslatorPreviewPopup:init()
    ...
    local fixed_height = top_border_size
        ...
        + padding_bottom
        + 2 * card_border_size
    local max_html_height = math_max(1, max_popup_height - fixed_height)
    local html_height = estimateHtmlHeight(self.html_body, content_width, self.doc_font_size, max_html_height)
    html_height = math_min(max_html_height, html_height + HTML_BOTTOM_SAFETY)
...
function TranslatorPreviewPopup:makeButtons(width)
    ...
    local button_count = math_max(1, #actions)
    local separator_count = math_max(0, button_count - 1)
    local available_button_width = math_max(1, width - BUTTON_SEPARATOR_WIDTH * separator_count)
...
function TranslatorPreview:getReaderUI()
    ...
    local ok, reader_ui_module = pcall(require, "apps/reader/readerui")

    if ok and reader_ui_module and reader_ui_module.instance then
        self.reader_ui = reader_ui_module.instance
        return self.reader_ui
    end
...
function TranslatorPreview:getSelectionBounds()
    ...
    local function addPosition(pos)
        ...
        if y and y >= 0 and y <= screen_height then
            top = top and math_min(top, y) or y
            bottom = bottom and math_max(bottom, y) or y
        end
    end
...
function TranslatorPreview:closeLanguageMenu()
    if self.language_menu then
        pcall(UIManager.close, UIManager, self.language_menu)
        self.language_menu = nil
    end
end
...
function TranslatorPreview:getTargetLanguageMenuItems(data)
    ...
    local ok, settings_menu = pcall(translator_self.genSettingsMenu, translator_self)
