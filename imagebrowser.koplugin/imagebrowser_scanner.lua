-- ImageBrowser scanner: finds reference-worthy images inside an EPUB.
--
...
-- (stored in ImageBrowser's sidecar) are invalidated on plugin upgrade.
M.VERSION = 5
...
local function dims_jpeg(d)
	if #d < 4 or d:byte(1) ~= 0xFF or d:byte(2) ~= 0xD8 then
		return nil
	end
	local i = 3
	while i + 3 <= #d do
		...
		-- skip fill bytes
		while d:byte(i + 1) == 0xFF and i + 1 < #d do
			i = i + 1
		end
...
local function svg_len(v)
	...
	if unit == "pt" then
		return math.floor(n * 96 / 72 + 0.5)
	end
	if unit == "in" then
		return math.floor(n * 96 + 0.5)
	end
	if unit == "cm" then
		return math.floor(n * 96 / 2.54 + 0.5)
	end
	if unit == "mm" then
		return math.floor(n * 9.6 / 2.54 + 0.5)
	end
...
function M.parse_opf(xml, opf_dir)
	...
	local spine = {}
	local spine_block = xml:match("<[sS][pP][iI][nN][eE][%s>].-</[sS][pP][iI][nN][eE]%s*>") or xml
	...
	if guide_block then
		for tag in guide_block:gmatch("<[rR][eE][fF][eE][rR][eE][nN][cC][eE][%s/][^>]*>") do
			...
			if href and rtype then
				local p = resolve_path(opf_dir, url_decode((href:gsub("#.*$", ""):gsub("%?.*$", ""))))
				guide[p] = rtype:lower()
			end
		end
	end
...
local function is_html_media(media, path)
	if media:find("html") or media:find("xml%+xhtml") then
		return true
	end
...
function M.meaningful_text(s)
	...
	if l:match("^%S+%.%w%w%w?%w?$") then
		return nil
	end -- "map01.png"
	if l:match("^images?%s*%d*$") or l:match("^img[%s_%-%d]*$") then
		return nil
	end
	if l:match("^photos?%s*%d*$") or l:match("^pictures?%s*%d*$") then
		return nil
	end
	if l:match("^picture%s*%d*$") or l:match("^illustration%s*%d*$") then
		return nil
	end
	if l:match("^cover") then
		return nil
	end
	if l:match("^%d+$") then
		return nil
...
function M.decorative_caption(s)
	...
	if
		l:match("%f[%a]logo%f[%A]")
		or l:find("back ad", 1, true)
		or l:match("%f[%a]advertisement%f[%A]")
		or l:match("%f[%a]advert%f[%A]")
	then
		return true
	end
	...
	return (
		...
		or l:match("^part%f[%W]")
		or l:match("^book%s+%w+$")
		or l:match("^a note%f[%W]")
		or l:match("^prologue")
		or l:match("^epilogue")
		or l:match("^interlude")
		or l:match("^introduction")
		or l:match("^foreword")
		or l:match("^preface")
		or l:match("^afterword")
		or l:match("^appendix")
		or l:match("^acknowledg")
		or l:match("^contents")
		or l:match("^table of contents")
		or l:match("^dedication")
		or l:match("^epigraph")
		or l:match("^author.?s note")
		or l:match("^book title")
		or l:match("^title page")
		or l:match("^half.?title")
		or l:match("^also by%f[%W]")
		or l:match("^by the same author")
	)
			and true
		or false
end
...
function M.decorative_name(path)
	...
	return (
		base:match("%f[%a]author%f[%A]")
		or base:match("%f[%a]logo%f[%A]")
		or base:match("%f[%a]publisher")
		or base:match("%f[%a]colophon")
		or base:match("%f[%a]copyright")
		or base:match("half.?title")
		or base:match("title.?page")
		or base:match("%f[%a]title%d*%.%w+$")
		or base:match("%f[%a]backad")
		or base:match("%f[%a]newsletter")
		or base:match("%f[%a]signup")
		or base:match("%f[%a]endpaper")
		or base:match("%f[%a]tp%f[%A]")
		or base:match("%f[%a]cvi%f[%A]")
		or base:match("%f[%a]cop%f[%A]")
		or base:match("%f[%a]adc%f[%A]")
		or base:match("%f[%a]ata%f[%A]")
	)
			and true
		or false
end
...
function M.figure_name(path)
	...
	return (
		base:match("^f%d+%-%d+%.")
		or base:match("^f%d+[a-z]?%.")
		or base:match("^fig%d")
		or base:match("^figure")
		or base:match("%f[%a]fig%d+%f[%A]")
	)
			and true
		or false
end
...
function M.weak_caption(s)
	...
	if M.decorative_caption(s) then
		return true
	end
	local l = trim(collapse_ws(s)):lower()
	local words = 0
			...
			if name then
				...
				if (name == "img" or name == "image") and body_depth then
					...
					for i = body_depth + 1, #stack do
						parts[#parts + 1] = stack[i].tag .. "[" .. stack[i].index .. "]"
					end
...
function M.extract_images(html)
	...
	local function add(tag, pos, src)
		...
		if src:match("^data:") or src:match("^%a+://") then
			return
		end
		local f = fig_at(pos)
		out[#out + 1] = {
...
function M.filter(images, level)
	...
	for _, img in ipairs(images) do
		...
		local captioned = ((img.caption ~= nil or img.in_figure) and not decorative) or M.figure_name(img.path)
		...
		elseif img.files_count > M.MAX_SPINE_FILES then
			...
			if img.width and img.height then
				...
				-- not flag
				if not reason and not captioned and img.spine_index <= M.FRONTMATTER_SPINE then
			...
			if not reason then
				if
					decorative
					or (
						M.weak_caption(img.caption)
						and (
							M.decorative_name(img.path)
							or (img.doc_path and M.decorative_name(img.doc_path))
							or M.chrome_role(img.doc_role)
						)
					)
				then
					reason = "decorative"
			...
			if not reason then
				...
				if not (w and h) then
					...
					if short > 0 and long / short > max_ratio then
						reason = "aspect"
					elseif short < t.short * relief or long < t.long * relief or w * h < t.area * relief * relief then
						reason = "small"
					end
				end
			end
