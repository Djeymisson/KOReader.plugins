--[[--
ImageBrowser: peek at maps, family trees and other reference images from anywhere
in the book, without losing your reading position.

EPUB-only (crengine): the book's HTML is parsed directly (see
imagebrowser_scanner.lua), which gives real pixel dimensions plus captions/alt
...
local scanner
do
	local ok, mod = pcall(dofile, _PLUGIN_DIR .. "/imagebrowser_scanner.lua")
	if ok then
		scanner = mod
	end
end

local SCOPE_KEY = "imagebrowser_scope" -- "read_so_far" | "whole_book"
...
local FILTER_KEY = "imagebrowser_filter"
-- Invert images while night mode is on (global setting).
local INVERT_KEY = "imagebrowser_invert_night"
local NAV_BUTTONS_KEY = "imagebrowser_nav_buttons" -- prev/next buttons, off by default
local CAPTIONS_KEY = "imagebrowser_captions" -- caption overlay, ON by default (nilOrTrue)
local TOP_MENU_KEY = "imagebrowser_top_menu_zone" -- tap top strip → KOReader top menu, ON by default (nilOrTrue)
local SHADOW_KEY = "imagebrowser_disable_shadow" -- drop the panel's gradient shadow, OFF by default (e-ink ghost source)
local CENTERED_POPUP_KEY = "imagebrowser_centered_popup" -- legacy centered-popup boolean
local POPUP_LAYOUT_KEY = "imagebrowser_popup_layout"
local POPUP_LAYOUT_LEFT_ROUNDED = "left_rounded"
local POPUP_LAYOUT_CENTERED_ROUNDED = "centered_rounded"
local POPUP_LAYOUT_CENTERED_SQUARE = "centered_square"
local GESTURE_TIP_KEY = "imagebrowser_gesture_tip_shown" -- one-time menu-open nudge to bind a gesture
...
local QUICK_ACTIONS_KEY = "imagebrowser_quick_actions"
local QUICK_ACTIONS = {
...
-- The layout setting deliberately exposes only valid combinations: the
-- left-side gallery always has rounded outer corners, while square corners
-- are available only on the centered popup. Fall back to the old boolean so
-- existing installations keep their selected position after upgrading.
local function _popup_layout()
	local layout = G_reader_settings:readSetting(POPUP_LAYOUT_KEY)
	if
		layout == POPUP_LAYOUT_LEFT_ROUNDED
		or layout == POPUP_LAYOUT_CENTERED_ROUNDED
		or layout == POPUP_LAYOUT_CENTERED_SQUARE
	then
		return layout
	end
	return G_reader_settings:isTrue(CENTERED_POPUP_KEY) and POPUP_LAYOUT_CENTERED_ROUNDED or POPUP_LAYOUT_LEFT_ROUNDED
end

local function _set_popup_layout(layout)
	if layout ~= POPUP_LAYOUT_CENTERED_ROUNDED and layout ~= POPUP_LAYOUT_CENTERED_SQUARE then
		layout = POPUP_LAYOUT_LEFT_ROUNDED
	end
	G_reader_settings:saveSetting(POPUP_LAYOUT_KEY, layout)
	-- Keep the old key in sync for downgrade compatibility.
	G_reader_settings:saveSetting(CENTERED_POPUP_KEY, layout ~= POPUP_LAYOUT_LEFT_ROUNDED)
end

local function _popup_is_centered(layout)
	return layout ~= POPUP_LAYOUT_LEFT_ROUNDED
end
...
-- fallback kicks in).
local ImageBrowserDots = Widget:extend({
	nb = 1,
	cur = 1,
...
function ImageBrowserDots:getSize()
	return Geom:new({
...
-- nb/cur/pitch are fixed for the lifetime of an instance (a new ImageBrowserDots
-- is built whenever any of them change — see _pillAvailWidth's caller), so
-- baking the dot row into a bitmap ONCE and reusing it on every repaint
-- (image switches, ⋯ menu open/close, night-mode toggles, ...) replaces a
-- per-pixel paint_dot recompute — itself a nested loop of individual
-- bb:paintRect calls — with a single opaque blit, the same pattern already
-- used by ImageBrowserPill/ImageBrowserBadge/ImageBrowserMoreButton for their chrome.
function ImageBrowserDots:paintTo(bb, x, y)
	local size = self:getSize()
	self.dimen = Geom:new({ x = x, y = y, w = size.w, h = self.height })
	if not self._bg_bb then
		-- dots always sit on the pill's solid black fill (ImageBrowserPill is
		-- never `inverted` when it wraps ImageBrowserDots), so an opaque
		-- black-filled buffer matches the surrounding pixels exactly and
		-- can be blitted opaquely instead of alpha-blended.
		self._bg_bb = Blitbuffer.new(size.w, self.height, Blitbuffer.TYPE_BB8)
		self._bg_bb:fill(Blitbuffer.Color8(0x00))
		local cy = math.floor(self.height / 2)
		local x0 = self.dot_r
		for i = 1, self.nb do
			local cx = x0 + (i - 1) * self.pitch
			paint_dot(self._bg_bb, cx, cy, self.dot_r, i == self.cur and 0xFF or 0x66, 0x00)
		end
	end
	bb:blitFrom(self._bg_bb, x, y, 0, 0, size.w, self.height)
end

function ImageBrowserDots:free()
	if self._bg_bb then
		self._bg_bb:free()
		self._bg_bb = nil
	end
...
local ImageBrowserEllipsis = Widget:extend({
	size = Screen:scaleBySize(18),
})

function ImageBrowserEllipsis:getSize()
	return Geom:new({ w = self.size, h = self.size })
end

-- Same rationale as ImageBrowserDots:paintTo: the glyph is static (three dots on
-- white), yet the ⋯ button repaints repeatedly (press/release feedback,
-- drawer redraws) — bake it once into an opaque buffer instead of rerunning
-- three per-pixel paint_dot passes on every repaint.
function ImageBrowserEllipsis:paintTo(bb, x, y)
	self.dimen = Geom:new({ x = x, y = y, w = self.size, h = self.size })
	if not self._bg_bb then
		self._bg_bb = Blitbuffer.new(self.size, self.size, Blitbuffer.TYPE_BB8)
		self._bg_bb:fill(Blitbuffer.Color8(0xFF))
		local r = math.max(2, math.floor(self.size / 9))
		local cx = math.floor(self.size / 2)
		local cy = math.floor(self.size / 2)
		paint_dot(self._bg_bb, cx - 3 * r, cy, r, 0x00, 0xFF)
		paint_dot(self._bg_bb, cx, cy, r, 0x00, 0xFF)
		paint_dot(self._bg_bb, cx + 3 * r, cy, r, 0x00, 0xFF)
	end
	bb:blitFrom(self._bg_bb, x, y, 0, 0, self.size, self.size)
end

function ImageBrowserEllipsis:free()
	if self._bg_bb then
		self._bg_bb:free()
		self._bg_bb = nil
	end
		...
		for px = 0, w - 1 do
			...
			if cov > 0 then
				...
				if no_fill then
					...
					local a = cov * (1 - t_in)
					if a > 0 then
						bb:setPixel(px, py, Blitbuffer.ColorRGB32(outline, outline, outline, math.floor(a * 255 + 0.5)))
					end
				else
					local g = math.floor(outline + t_in * (fill - outline) + 0.5)
					bb:setPixel(px, py, Blitbuffer.ColorRGB32(g, g, g, math.floor(cov * 255 + 0.5)))
				end
			end
		end
	end
...
local ImageBrowserPill = WidgetContainer:extend({
...
function ImageBrowserPill:init()
	self[1] = self.inner
end

function ImageBrowserPill:getSize()
...
function ImageBrowserPill:paintTo(bb, x, y)
	local size = self:getSize()
...
function ImageBrowserPill:free(...)
	if self._bg_bb then
		self._bg_bb:free()
		self._bg_bb = nil
	end
...
local ImageBrowserBadge = Widget:extend({
	num = 1,
...
function ImageBrowserBadge:init()
...
function ImageBrowserBadge:getSize()
	return Geom:new({ w = self._w, h = self.height })
end

function ImageBrowserBadge:paintTo(bb, x, y)
	self.dimen = Geom:new({ x = x, y = y, w = self._w, h = self.height })
	if not self._bg_bb then
		self._bg_bb = make_rounded_stencil(self._w, self.height, self.radius, self.stroke, 0xFF, 0x00)
	end
	bb:alphablitFrom(self._bg_bb, x, y, 0, 0, self._w, self.height)
	local ts = self._txt:getSize()
	self._txt:paintTo(bb, x + math.floor((self._w - ts.w) / 2), y + math.floor((self.height - ts.h) / 2))
end

function ImageBrowserBadge:free()
	if self._bg_bb then
		self._bg_bb:free()
		self._bg_bb = nil
	end
...
local ImageBrowserMoreButton = Widget:extend({
...
function ImageBrowserMoreButton:getSize()
	return Geom:new({ w = self.size, h = self.size })
end

function ImageBrowserMoreButton:paintTo(bb, x, y)
	self.dimen = Geom:new({ x = x, y = y, w = self.size, h = self.size })
	if not self._bg_bb then
		...
		self._bg_bb = make_rounded_stencil(
			self.size,
			self.size,
			self.radius,
			self.stroke,
	...
	if self._icon_bb then
		bb:alphablitFrom(
			...
			0,
			0,
			self._icon_bb:getWidth(),
			self._icon_bb:getHeight()
		)
	else
		if not self._icon then
			self._icon = ImageBrowserEllipsis:new({})
		end
		local isz = self._icon:getSize()
		self._icon:paintTo(bb, x + math.floor((self.size - isz.w) / 2), y + math.floor((self.size - isz.h) / 2))
	end
	if self.inverted then
		...
		for yy = 0, self.size - 1 do
			for xx = 0, self.size - 1 do
				...
				if a > 127 then
					bb:setPixel(x + xx, y + yy, bb:getPixel(x + xx, y + yy):getColorRGB32():invert())
				end
			end
		end
	end
...
function ImageBrowserMoreButton:free()
	if self._bg_bb then
		self._bg_bb:free()
		self._bg_bb = nil
	end
...
-- Caption overlay: the image's caption shown as a centered tab near the
-- top edge of the viewer — white fill, black text, with the bottom corners
-- rounded so it sits flush against the panel's top edge. Painted in DAY
-- polarity, so night mode's framebuffer inversion flips it to a black tab
-- with white text automatically — the wanted look holds both ways with no
-- per-mode branching. Truncates to max_width.
local ImageBrowserCaption = Widget:extend({
	text = "",
	max_width = 0,
...
function ImageBrowserCaption:init()
	...
	self._text = TextBoxWidget:new({
		...
		fgcolor = Blitbuffer.COLOR_BLACK,
		width = box_w,
		alignment = self.align or "center",
		-- height omitted -> auto, grows with the number of wrapped lines
	})
end

function ImageBrowserCaption:getSize()
...
-- Solid white tab with the caption text baked in. TextBoxWidget:paintTo
-- blits an opaque rectangle, so the text is composited FIRST and the bottom
-- corners are carved LAST — otherwise the opaque text box would refill the
-- rounded corners. Opaque everywhere except the carved corners, so it reads
-- as a clean-edged tab over the image and inverts to solid black at night.
function ImageBrowserCaption:_buildBg(w, h)
	...
	local function carve_corner(corner)
		local r = self.radius
		if r <= 0 then
			return
		end
		local x0, x1, y0, y1, cx, cy
		if corner == "bottom_right" then
			x0, x1 = math.floor(w - r), w - 1
			y0, y1 = math.floor(h - r), h - 1
			cx, cy = w - r, h - r
		elseif corner == "bottom_left" then
			x0, x1 = 0, math.ceil(r) - 1
			y0, y1 = math.floor(h - r), h - 1
			cx, cy = r, h - r
		else
			return
	...
	carve_corner("bottom_right")
	if self.centered then
		carve_corner("bottom_left")
	end
end

function ImageBrowserCaption:paintTo(bb, x, y)
...
function ImageBrowserCaption:free()
	if self._text then
		self._text:free()
	end
	if self._bg_bb then
...
local ImageBrowserTextButton = Widget:extend({
	text = "",
	bold = false,
...
function ImageBrowserTextButton:init()
...
function ImageBrowserTextButton:getSize()
	return Geom:new({ w = self._w, h = self.height })
end
...
function ImageBrowserTextButton:setWidth(w)
...
function ImageBrowserTextButton:paintTo(bb, x, y)
	self.dimen = Geom:new({ x = x, y = y, w = self._w, h = self.height })
	if not self._bg_bb then
		self._bg_bb = make_rounded_stencil(self._w, self.height, self.radius, self.stroke, 0xFF, 0x00)
	end
	...
	if self.inverted then
		...
		-- stencil's alpha), same trick as ImageBrowserMoreButton
...
function ImageBrowserTextButton:free()
	if self._bg_bb then
		self._bg_bb:free()
		self._bg_bb = nil
	end
...
local ImageBrowserMenuRow = Widget:extend({
	text = "",
...
function ImageBrowserMenuRow:init()
	self._text_wg = TextWidget:new({
		text = self.text,
		face = Font:getFace("cfont", 15),
		bold = true,
...
function ImageBrowserMenuRow:getSize()
	return Geom:new({ w = self.width, h = self.height })
end

function ImageBrowserMenuRow:paintTo(bb, x, y)
	...
	if self.icon_bb then
		bb:alphablitFrom(
			...
			0,
			0,
			self.icon_bb:getWidth(),
			self.icon_bb:getHeight()
		)
	elseif self.lead_wg then
		local lsz = self.lead_wg:getSize()
		self.lead_wg:paintTo(
			bb,
	...
	local tsz = self._text_wg:getSize()
	self._text_wg:paintTo(bb, x + self.pad_left + self.icon_col, y + math.floor((self.height - tsz.h) / 2))
end

function ImageBrowserMenuRow:free()
	if self._text_wg then
		self._text_wg:free()
	end
	if self.lead_wg then
		self.lead_wg:free()
	end
end
...
local ImageBrowserCard = WidgetContainer:extend({
	radius = Screen:scaleBySize(9),
	stroke = Screen:scaleBySize(2),
	outline = 0x00, -- black border, matching the old FrameContainer
})

function ImageBrowserCard:getSize()
	return self[1]:getSize()
end

function ImageBrowserCard:paintTo(bb, x, y)
	...
	if not self._fill_bb or self._bg_w ~= sz.w or self._bg_h ~= sz.h then
		if self._fill_bb then
			self._fill_bb:free()
		end
		if self._ring_bb then
			self._ring_bb:free()
		end
		self._bg_w, self._bg_h = sz.w, sz.h
		-- solid white rounded rect (outline == fill, so no visible edge yet)
		self._fill_bb = make_rounded_stencil(sz.w, sz.h, self.radius, self.stroke, 0xFF, 0xFF)
		-- border-only ring, laid over the content afterwards
		self._ring_bb = make_rounded_stencil(sz.w, sz.h, self.radius, self.stroke, nil, self.outline)
	end
...
function ImageBrowserCard:free(full)
	if self._fill_bb then
		self._fill_bb:free()
		self._fill_bb = nil
	end
...
local ImageBrowserPopupMenu = InputContainer:extend({
...
function ImageBrowserPopupMenu:init()
	self._icon_bbs = {}
	...
	for i, it in ipairs(self.items) do
		...
		if it.icon then
			local ok, ibb = pcall(RenderImage.renderSVGImageFile, RenderImage, it.icon, self.icon_size, self.icon_size)
			if ok and ibb then
				icon_bb = ibb
		...
		local row = ImageBrowserMenuRow:new({
			text = it.text,
			icon_bb = icon_bb,
			lead_wg = lead_wg,
			width = row_w,
...
function ImageBrowserPopupMenu:dismiss()
...
function ImageBrowserPopupMenu:onTap(_, ges)
...
function ImageBrowserPopupMenu:onClose()
	self:dismiss()
	return true
end

function ImageBrowserPopupMenu:onCloseWidget()
	for _, ibb in ipairs(self._icon_bbs) do
		ibb:free()
	end
	self._icon_bbs = {}
...
-- Layout: by default, a full-height drawer anchored to the LEFT screen
-- edge. The same viewer can be drawn as an 80% × 80% popup centered on the
-- screen, with rounded or square corners. The panel is painted from a stencil
-- in _paintPanel so both layouts keep the existing e-ink refresh behavior.

local ImageBrowserViewer = ImageViewer:extend({
	...
	-- Panel metrics from the design (design px == px at the reference DPI)
	panel_ratio = 505 / 630, -- left drawer width / screen width
	popup_ratio = 0.80, -- centered popup width and height
	panel_vgap = 0, -- retained for the drawer layout
	panel_border = Screen:scaleBySize(2),
	-- Keep the rounded style visually aligned with LookupPreview cards.
	panel_radius = Screen:scaleBySize(14),
	...
	-- zoomed on double-taps outside the panel. Must be an explicit true,
...
function ImageBrowserViewer:init()
...
function ImageBrowserViewer:_prefFor(i)
...
-- same lifecycle, with either a left-anchored drawer or a centered popup.
-- The dot pill and ⋯ button are OVERLAID on the image instead of stacked
-- below it.
function ImageBrowserViewer:update()
	self:_clean_image_wg()
	local orig_dimen = self.main_frame.dimen

	self._popup_layout = _popup_layout()
	self._centered_popup = _popup_is_centered(self._popup_layout)
	self._rounded_popup = self._popup_layout ~= POPUP_LAYOUT_CENTERED_SQUARE
	if self._centered_popup then
		self._panel_w = math.floor(Screen:getWidth() * self.popup_ratio + 0.5)
		self._panel_h = math.floor(Screen:getHeight() * self.popup_ratio + 0.5)
		...
		-- The drawer is borderless on the left because it is flush with the
		-- screen edge; the other three sides retain the normal border.
		self.width = self._panel_w - self.panel_border
	end
	-- self.width/height are what the inherited zoom/pan code sizes against.
	self.height = self._panel_h - 2 * self.panel_border

	while table.remove(self.frame_elements) do
	end
	...
	-- edge than before (the buttons also grew 2px, see ImageBrowserMoreButton)
	...
	if self._nav_prev_frame then
		self._nav_prev_frame:free()
	end
	if self._nav_next_frame then
		self._nav_next_frame:free()
	end
	self._nav_prev_frame, self._nav_next_frame = nil, nil
	local nav = G_reader_settings:isTrue(NAV_BUTTONS_KEY) and self._images_list and (self._images_list_nb or 1) > 1
	...
	if nav then
		self._nav_prev_frame = ImageBrowserMoreButton:new({
		...
		self._nav_next_frame = ImageBrowserMoreButton:new({
	...
	if self._gallery_mode then
		self._close_frame = ImageBrowserTextButton:new({
			text = _("Back"),
			bold = true,
			icon = _PLUGIN_DIR .. "/assets/back.svg",
		})
		local size = self._close_frame:getSize()
		local x = self._nav_next_frame and (self._nav_next_frame.overlap_offset[1] - btn_gap - size.w)
	...
	if self._pill_frame then
		...
		local bottom_inset = (self:_isOverFit() or self._gallery_mode) and btn_inset or Screen:scaleBySize(25)
		...
		if self._nav_prev_frame and self._nav_prev_frame.overlap_offset then
			left_bound = self._nav_prev_frame.overlap_offset[1] + self._nav_prev_frame.size
		end
		local right_bound = image_area_w
		for _, f in ipairs({ self._more_frame, self._close_frame, self._nav_next_frame }) do
		...
		if self._gallery_mode and self._pill_frame.setWidth then
			...
			local pill_left = self._nav_prev_frame and (left_bound + btn_gap) or left_bound
			local pill_right = right_bound - btn_gap
			self._pill_frame:setWidth(pill_right - pill_left)
			self._pill_frame.overlap_offset = {
				pill_left,
				self.height - self._pill_frame:getSize().h - bottom_inset,
			}
		else
	...
	-- caption overlay, centered at the top of the image (toggleable, on by default)
	...
	if G_reader_settings:nilOrTrue(CAPTIONS_KEY) and not self._gallery_mode then
		...
		if caption and caption ~= "" then
			self._caption_wg = ImageBrowserCaption:new({
				text = caption,
				max_width = image_area_w - 2 * Screen:scaleBySize(32),
				centered = true,
				align = "center",
			})
			local csz = self._caption_wg:getSize()
			self._caption_wg.overlap_offset = {
				math.max(0, math.floor((image_area_w - csz.w) / 2)),
				0,
			}
	...
	-- main_frame is transparent; the panel body and shadow are painted by
	-- the _paintPanel hook. The centered popup has a border on every side,
	-- while the drawer remains borderless where it touches the left edge.
	self.main_frame.background = nil
	self.main_frame.radius = nil
	self.main_frame.bordersize = 0
	self.main_frame.padding = 0
	self.main_frame.padding_left = self._centered_popup and self.panel_border or 0
	self.main_frame.padding_right = self.panel_border
	self.main_frame.padding_top = self.panel_border
	self.main_frame.padding_bottom = self.panel_border
	...
	-- ImageViewer wraps main_frame in a CenterContainer. An explicit center
	-- alignment gives the popup its position; nil preserves the drawer's
	-- existing left-edge placement.
	self[1].align = self._centered_popup and "center" or nil
	...
	if flash_switch and not fast then
		wfm_mode = "full"
	end
	...
	UIManager:setDirty(self, function()
		...
		if not self.main_frame.dimen then
			return
		end
...
-- Paints the active layout at (x, y): first the dithered dot-pattern shadow,
-- then the panel body from a cached stencil. The drawer rounds its right
-- corners, the centered rounded popup rounds all four, and the centered
-- square popup returns no corner specs. Blending is safe against accumulation
-- because self.alpha makes UIManager repaint the windows below us first (see
-- the class comment).
function ImageBrowserViewer:_cornerSpecs()
	if not self._rounded_popup then
		return {}
	end
	local r, w, h = self.panel_radius, self._panel_w, self._panel_h
	local specs = {
		{ x = w - r, y = 0, cx = 0, cy = r },
		{ x = w - r, y = h - r, cx = 0, cy = 0 },
	}
	if self._centered_popup then
		table.insert(specs, 1, { x = 0, y = 0, cx = r, cy = r })
		table.insert(specs, 2, { x = 0, y = h - r, cx = r, cy = 0 })
	end
	return specs
end

function ImageBrowserViewer:_freeCornerBuffers()
	for _, key in ipairs({ "_under_corner_bbs", "_corner_bbs" }) do
		local buffers = self[key]
		if buffers then
			for _, corner in ipairs(buffers) do
				corner:free()
			end
			self[key] = nil
		end
...
function ImageBrowserViewer:_shadowMetrics(night)
	...
	return {
		right_width = width,
		right_overlap = self._centered_popup and self.popup_shadow_overlap or self.shadow_overlap,
		bottom_height = self._centered_popup and self.popup_bottom_shadow_height or 0,
		bottom_overlap = self._centered_popup and self.popup_bottom_shadow_overlap or 0,
	}
end
...
function ImageBrowserViewer:_paintPanel(bb, x, y)
	...
	local swidth = shadow_metrics.right_width
	...
	local function shadowLevel(pos, length, overlap)
		local t = (pos + 0.5) / length
		local orig_level = speak * origFrac(t)
		local vis0 = overlap / length
		local bump
		if t <= vis0 then
			bump = 1
		else
		...
		then
			...
			for j = 0, bottom_height - 1 do
				...
				for i = 0, bottom_w - 1 do
					...
					-- both directions so the two short fades meet cleanly.
					if j < bottom_overlap and i >= w - right_overlap then
						level = 0
					elseif i >= w then
	...
	-- interior-only repaints. The drawer has two rounded corners, the rounded
	-- popup has four, and the square popup has none.
	local cr = self.panel_radius
	local cpy = y + self.panel_vgap
	local corner_specs = self:_cornerSpecs()
	if not self._under_corner_bbs or #self._under_corner_bbs ~= #corner_specs then
		self:_freeCornerBuffers()
		self._under_corner_bbs = {}
		for i = 1, #corner_specs do
			self._under_corner_bbs[i] = Blitbuffer.new(cr, cr, Blitbuffer.TYPE_BBRGB32)
		end
	end
	local ucb = self._under_corner_bbs
	for i, spec in ipairs(corner_specs) do
		if skip_shadow then
			bb:blitFrom(ucb[i], x + spec.x, cpy + spec.y, 0, 0, cr, cr)
		else
			-- Match the framebuffer's inverse flag so these copies use the
			-- C blitter on software-inverted devices.
			ucb[i]:setInverse(render_inv and 1 or 0)
			ucb[i]:blitFrom(bb, 0, 0, x + spec.x, cpy + spec.y, cr, cr)
		end
	end

	if
		not self._panel_bb
		or self._panel_bb:getWidth() ~= w
		or self._panel_bb:getHeight() ~= h
		or self._panel_night ~= skey
		or self._panel_centered ~= self._centered_popup
		or self._panel_rounded ~= self._rounded_popup
	then
		if self._panel_bb then
			self._panel_bb:free()
		end
		self._panel_night = skey
		self._panel_centered = self._centered_popup
		self._panel_rounded = self._rounded_popup
		...
		-- AA corner arcs: body inside, border ring, transparent outside.
		for _, spec in ipairs(corner_specs) do
			for pyy = 0, r - 1 do
				for pxx = 0, r - 1 do
					local d = math.sqrt((pxx + 0.5 - spec.cx) ^ 2 + (pyy + 0.5 - spec.cy) ^ 2)
					local cov = math.min(math.max(r - d + 0.5, 0), 1)
					local t_in = math.min(math.max((r - bw) - d + 0.5, 0), 1)
					local g = math.floor(edge + t_in * (body - edge) + 0.5)
					self._panel_bb:setPixel(
						spec.x + pxx,
						spec.y + pyy,
...
function ImageBrowserViewer:_saveCorners(bb, x, py)
	local r, bw = self.panel_radius, self.panel_border
	local specs = self:_cornerSpecs()
	if not self._corner_bbs or #self._corner_bbs ~= #specs then
		if self._corner_bbs then
			for _, corner in ipairs(self._corner_bbs) do
				corner:free()
			end
		end
		self._corner_bbs = {}
		for i = 1, #specs do
			self._corner_bbs[i] = Blitbuffer.new(r, r, Blitbuffer.TYPE_BBRGB32)
		end
	end
	local keep_r = r - bw - self.image_padding
	for i, spec in ipairs(specs) do
		local cbb = self._corner_bbs[i]
		cbb:blitFrom(bb, 0, 0, x + spec.x, py + spec.y, r, r)
		for pyy = 0, r - 1 do
			for pxx = 0, r - 1 do
				local d = math.sqrt((pxx + 0.5 - spec.cx) ^ 2 + (pyy + 0.5 - spec.cy) ^ 2)
				...
				if t_in > 0 then
					...
					cbb:setPixel(pxx, pyy, Blitbuffer.ColorRGB32(c.r, c.g, c.b, math.floor((1 - t_in) * 255 + 0.5)))
				end
			end
		end
	end
...
function ImageBrowserViewer:_restoreCorners(bb, x, y)
	if not self._corner_bbs then
		return
	end
	local py = y + self.panel_vgap
	for i, spec in ipairs(self:_cornerSpecs()) do
		bb:alphablitFrom(self._corner_bbs[i], x + spec.x, py + spec.y, 0, 0, self.panel_radius, self.panel_radius)
	end
end
...
function ImageBrowserViewer:onSetRotationMode(rotation)
...
function ImageBrowserViewer:onCloseWidget()
	...
	if self._gallery_head_wgs then
		for _, w in ipairs(self._gallery_head_wgs) do
			w:free()
		end
		self._gallery_head_wgs = nil
	end
	if self._gallery_badges then
		for _, b in ipairs(self._gallery_badges) do
			b:free()
		end
	...
	UIManager:setDirty(nil, function()
		...
		-- Cover the active right and bottom shadows. With shadow disabled,
		-- keep the region to the panel so a promoted/flash refresh never
	...
	-- the count is still ImageBrowser's (0) and can't flash from the reader's total.
...
function ImageBrowserViewer:_new_image_wg()
...
function ImageBrowserViewer:_getHiRes()
	if not self.hires_decode then
		return nil
	end
	if self._hi_bb == false then
		return nil
	end
	if self._hi_bb then
		return self._hi_bb
...
function ImageBrowserViewer:_resetHiRes()
...
function ImageBrowserViewer:_buildPill()
	if self._pill_frame then
	...
	if self._gallery_mode then
		...
		if self:_hasIgnoredTab() then
			...
			self._pill_frame = ImageBrowserTextButton:new({ text = label, bold = true })
		end
		return
	end
	if self:_isOverFit() then
		...
		self._pill_frame = ImageBrowserTextButton:new({
			text = _("Reset"),
			bold = true,
	...
	if not (self._images_list and self._images_list_nb > 1) then
		return
	end
	...
	local dot_r = ImageBrowserDots.dot_r
	local natural_pitch = ImageBrowserDots.pitch
	local min_pitch = 2 * dot_r + Screen:scaleBySize(2)
	local budget = self:_pillAvailWidth() - 2 * ImageBrowserPill.padding_h
	...
	if pitch >= min_pitch then
		local inner = ImageBrowserDots:new({
			nb = nb,
			cur = self._images_list_cur or 1,
			pitch = math.floor(pitch),
		})
		self._pill_dots = inner
		self._pill_frame = ImageBrowserPill:new({ inner = inner })
	else
		...
		self._pill_frame = ImageBrowserPill:new({
			inverted = true,
...
function ImageBrowserViewer:_pillAvailWidth()
	local image_area_w = self.width - self.image_right_gap
	local btn_inset = Screen:scaleBySize(16)
	local btn_gap = Screen:scaleBySize(10)
	local btn_size = ImageBrowserMoreButton.size
	local nav = G_reader_settings:isTrue(NAV_BUTTONS_KEY) and self._images_list and (self._images_list_nb or 1) > 1
	local more_left
	if nav then
...
function ImageBrowserViewer:_buildMoreButton()
	self._more_frame = ImageBrowserMoreButton:new({})
end
...
function ImageBrowserViewer:_tabList()
	if self._gallery_tab == "ignored" then
		return self.ignored_list, self.ignored_metas, self.ignored_metas and #self.ignored_metas or 0
	end
	return self.shown_list, self.shown_metas, self.shown_metas and #self.shown_metas or 0
end

function ImageBrowserViewer:_ignoredCount()
...
function ImageBrowserViewer:_hasIgnoredTab()
	return self:_ignoredCount() > 0
end

function ImageBrowserViewer:_switchGalleryTab(tab)
...
function ImageBrowserViewer:_enterGallery(page, tab)
...
function ImageBrowserViewer:_exitGallery(idx)
...
function ImageBrowserViewer:_galleryHit(pos)
	if not self._gallery_cells then
		return nil
	end
...
function ImageBrowserViewer:_openMoveMenu(cell, pos)
	...
	local menu
	menu = ImageBrowserPopupMenu:new({
		...
		anchor = function()
			local w = menu.movable and menu.movable.dimen and menu.movable.dimen.w or 0
...
function ImageBrowserViewer:_galleryLayout()
	...
	for i = 1, nb or 1 do
		...
		local best = 1
		for c = 2, cols do
			if heights[c] < heights[best] then
				best = c
			end
		end
...
-- in lockstep.
function ImageBrowserViewer:_headMetrics()
	if self._head_metrics then
...
function ImageBrowserViewer:_galleryMetrics()
...
function ImageBrowserViewer:_galleryGo(delta)
	local p = math.min(math.max((self._gallery_page or 1) + delta, 1), self:_galleryPages())
...
function ImageBrowserViewer:_thumb(i, w, h)
	...
	local bb
	if s < 1 then
		bb = RenderImage:scaleBlitBuffer(
			src,
			math.max(1, math.floor(bw * s + 0.5)),
			math.max(1, math.floor(bh * s + 0.5)),
			own
		)
	else
...
function ImageBrowserViewer:_buildGallery()
	local layout = self:_galleryLayout()
	...
	local active_is_primary = (self._gallery_tab or "shown") == (self.primary_tab or "shown")
	...
	for _, c in ipairs(layout.pages[self._gallery_page] or {}) do
		local bb = self:_thumb(c.idx, c.w - 2 * m.inset, c.h - 2 * m.inset)
		if bb then
			...
			local is_cur = active_is_primary and c.idx == (self._images_list_cur or 1)
			local cell = CenterContainer:new({
				dimen = Geom:new({ w = c.w, h = c.h }),
				FrameContainer:new({
					bordersize = is_cur and Screen:scaleBySize(2) or Screen:scaleBySize(1),
					color = is_cur and Blitbuffer.COLOR_BLACK or Blitbuffer.COLOR_GRAY,
			...
			table.insert(self._gallery_cells, { x = c.x, y = c.y, w = c.w, h = c.h, idx = c.idx })
			...
			if not on_ignored_tab then
				local badge = ImageBrowserBadge:new({ num = c.idx })
...
function ImageBrowserViewer:_hasQuickActions()
	for _, d in ipairs(QUICK_ACTIONS) do
...
function ImageBrowserViewer:_showMoreMenu()
	...
	if _quick_enabled("mode") then
		items[#items + 1] = {
			...
			text = self.scope == "whole_book" and _("Mode: All images") or _("Mode: Images up to here"),
			icon = _PLUGIN_DIR .. "/assets/mode.svg",
			callback = function()
				if self.on_toggle_scope then
	...
	if _quick_enabled("restore") and self.hidden_count and self.hidden_count() > 0 then
		items[#items + 1] = {
			text = _("Restore ignored images"),
			icon = _PLUGIN_DIR .. "/assets/restore.svg",
			callback = function()
				if self.on_restore_hidden then
					self.on_restore_hidden()
				end
			end,
		}
	end
	...
	if _quick_enabled("captions") then
		items[#items + 1] = {
			...
			callback = function()
				self:_toggleCaptions()
			end,
		}
	end
	if _quick_enabled("invert") then
		items[#items + 1] = {
			-- checkbox drawn in the icon column (see ImageBrowserMenuRow),
	...
	menu = ImageBrowserPopupMenu:new({
		...
		anchor = function()
			...
			return Geom:new({ x = d.x + d.w - w, y = d.y - gap, w = 0, h = d.h }), true
		end,
	})
	-- when the menu closes, also repaint the ⋯ button so its pressed
	-- (inverted) state clears
	menu._restore_region = self._more_frame and self._more_frame.dimen and self._more_frame.dimen:copy()
	menu.on_dismiss = function()
		if self._more_frame then
			self._more_frame.inverted = nil
		end
	end
...
function ImageBrowserViewer:_showInBook()
	local meta = self.image_metas and self.image_metas[self._images_list_cur or 1]
...
function ImageBrowserViewer:_rotateCurrent()
	self:_setRotation(((self._cur_rotation or 0) - 90) % 360)
end

function ImageBrowserViewer:_setRotation(rotation)
	...
	if meta and self.set_pref then
		self.set_pref(meta, "rotation", self._cur_rotation ~= 0 and self._cur_rotation or nil)
	end
	self:update()
end

function ImageBrowserViewer:_toggleInvert()
	local cur = self._images_list_cur or 1
...
function ImageBrowserViewer:_togglePrevNext()
	G_reader_settings:saveSetting(NAV_BUTTONS_KEY, not G_reader_settings:isTrue(NAV_BUTTONS_KEY))
	self:update()
end

function ImageBrowserViewer:_toggleCaptions()
	G_reader_settings:saveSetting(CAPTIONS_KEY, not G_reader_settings:nilOrTrue(CAPTIONS_KEY))
	self:update()
end
...
function ImageBrowserViewer:_checkDoubleTap(ges)
	local now = time.now()
	...
	if
		lt
		and now - lt.time < time.ms(350)
		and math.abs(ges.pos.x - lt.x) <= slop
		and math.abs(ges.pos.y - lt.y) <= slop
	then
		self._last_tap = nil
		self:onImageBrowserDoubleTap(nil, ges)
	end
end
...
function ImageBrowserViewer:onImageBrowserDoubleTap(_, ges)
	...
	if wg and ges and ges.pos then
		...
		self._center_x_ratio, self._center_y_ratio = wg:getPanByCenterRatio(ges.pos.x - cx, ges.pos.y - cy)
	end
...
function ImageBrowserViewer:_flashButton(frame, action)
...
function ImageBrowserViewer:_inTopMenuZone(pos)
...
-- elsewhere outside the panel closes; on the ⋯ button opens the menu.
-- Image switching is swipe-only (or the optional buttons), which leaves
-- the rest of the image as a double-tap zoom surface.
function ImageBrowserViewer:onTap(_, ges)
	-- In popup mode, every tap outside the card closes it immediately,
	-- including taps in KOReader's top-menu zone. This preserves modal popup
	-- behavior while leaving the drawer's original top-zone handling intact.
	local outside = self.main_frame.dimen and ges.pos:notIntersectWith(self.main_frame.dimen)
	if self._centered_popup and outside then
		self:onClose()
		return true
	end
	-- Respect KOReader's own top-of-screen menu trigger: a tap in that
	-- zone opens ONLY the top menu, over the still-open Image Browser (the ⋯
	...
	if self.on_show_menu and G_reader_settings:nilOrTrue(TOP_MENU_KEY) and self:_inTopMenuZone(ges.pos) then
		self.on_show_menu()
		return true
	end
	if outside then
	...
	if
		self._gallery_mode
		and self._close_frame
		and self._close_frame.dimen
		and ges.pos:intersectWith(self._close_frame.dimen)
	then
	...
	if
		not self._gallery_mode
		and self._more_frame
		and self._more_frame.dimen
		and ges.pos:intersectWith(self._more_frame.dimen)
	then
	...
	if self._nav_prev_frame and self._nav_prev_frame.dimen and ges.pos:intersectWith(self._nav_prev_frame.dimen) then
	...
	if self._nav_next_frame and self._nav_next_frame.dimen and ges.pos:intersectWith(self._nav_next_frame.dimen) then
	...
	if self._gallery_mode then
		...
		if self._pill_frame and self._pill_frame.dimen and ges.pos:intersectWith(self._pill_frame.dimen) then
			self:_flashButton(self._pill_frame, function()
				self:_switchGalleryTab(self._gallery_tab == "ignored" and "shown" or "ignored")
			end)
			return true
		end
...
function ImageBrowserViewer:onShowNextImage()
	if self._gallery_mode then
		self:_galleryGo(1)
		return true
	end
...
function ImageBrowserViewer:onShowPrevImage()
	if self._gallery_mode then
		self:_galleryGo(-1)
		return true
	end
...
function ImageBrowserViewer:switchToImageNum(image_num)
...
function ImageBrowserViewer:onSwipe(arg, ges)
	if self._gallery_mode then
...
-- drawer mid-pan — the "panning sometimes just closes ImageBrowser" bug. ImageBrowser
-- closes by tapping outside the panel instead, so swallow multiswipes here.
function ImageBrowserViewer:onMultiSwipe(_, ges)
	return true
end
...
function ImageBrowserViewer:onHold(_, ges)
	if self._gallery_mode then
		local cell = self:_galleryHit(ges.pos)
		if cell then
			self:_openMoveMenu(cell, ges.pos)
		end
		return true
	end
...
function ImageBrowserViewer:onPan(arg, ges)
...
function ImageBrowserViewer:_isOverFit()
	if self.scale_factor == 0 then
		return false
	end
...
function ImageBrowserViewer:_computeFitScaleFactor()
...
function ImageBrowserViewer:_refreshScaleFactor()
	if self._gallery_mode then
...
function ImageBrowserViewer:_applyNewScaleFactor(new_factor)
	if self._gallery_mode then
		return
	end
	...
	if ceil and new_factor > ceil then
		new_factor = ceil
	end
...
function ImageBrowserViewer:_nativeScale()
	local lo = self.image
	if not lo or not lo.getWidth then
		return nil
	end
	...
	if not nat_w or nat_w <= lo_w then
		return 1.0
	end
	return nat_w / lo_w
end
...
-- target land here.
function ImageBrowserViewer:_maxScale()
...
function ImageBrowserViewer:panBy(x, y)
	local wg = self._image_wg
	if not wg or not wg._bb then
		return
	end
	local cx = (x + wg._offset_x + wg.width / 2) / wg._bb_w
	local cy = (y + wg._offset_y + wg.height / 2) / wg._bb_h
	cx = math.min(math.max(cx, 0.5 - wg._max_off_center_x_ratio), 0.5 + wg._max_off_center_x_ratio)
	cy = math.min(math.max(cy, 0.5 - wg._max_off_center_y_ratio), 0.5 + wg._max_off_center_y_ratio)
	local ox = math.floor(cx * wg._bb_w - wg.width / 2)
	local oy = math.floor(cy * wg._bb_h - wg.height / 2)
	if ox == wg._offset_x and oy == wg._offset_y then
		return
	end
...
function ImageBrowserViewer:_hideCurrentImage()
...
local ImageBrowser = WidgetContainer:extend({
	name = "imagebrowser",
	-- also load in the file manager, so the Tools menu entry is always
	-- there; book-dependent actions answer with "No book is open." via
	-- _supportedReason
	is_doc_only = false,
})

function ImageBrowser:onDispatcherRegisterActions()
	Dispatcher:registerAction("imagebrowser_show", {
		category = "none",
		event = "ImageBrowserShow",
		title = _("Open Image Browser"),
		reader = true,
	})
end

function ImageBrowser:init()
	self:onDispatcherRegisterActions()
	self.ui.menu:registerToMainMenu(self)
end

function ImageBrowser:onImageBrowserShow()
	self:showViewer()
	return true
end

function ImageBrowser:onCloseDocument()
	-- the decoded-bitmap cache slot (see showViewer) is per book
	if self._bb_cache then
		if self._bb_cache.bb then
			self._bb_cache.bb:free()
		end
		self._bb_cache = nil
	end
end
...
function ImageBrowser:getScope()
	return G_reader_settings:readSetting(SCOPE_KEY) or "read_so_far"
end

function ImageBrowser:getFilterLevel()
	return G_reader_settings:readSetting(FILTER_KEY) == "all" and "all" or "balanced"
end

function ImageBrowser:_hiddenPaths()
	return (self.ui.doc_settings and self.ui.doc_settings:readSetting("imagebrowser_hidden")) or {}
end
...
-- The inverse of imagebrowser_hidden; a path in both means "shown" wins via the
-- partition in showViewer (add-back clears hidden and sets forced together).
function ImageBrowser:_forcedPaths()
	return (self.ui.doc_settings and self.ui.doc_settings:readSetting("imagebrowser_forced")) or {}
end

-- Per-image, per-book viewer preferences: { [path] = {rotation=90} }
function ImageBrowser:_imgPrefs()
	return (self.ui.doc_settings and self.ui.doc_settings:readSetting("imagebrowser_img_prefs")) or {}
end

function ImageBrowser:_setImgPref(path, key, value)
	...
	self.ui.doc_settings:saveSetting("imagebrowser_img_prefs", all)
...
function ImageBrowser:_hiddenCount()
	local n = 0
	for _ in pairs(self:_hiddenPaths()) do
		n = n + 1
	end
	return n
end
...
function ImageBrowser:_supportedReason()
	local doc = self.ui and self.ui.document
	if not doc or not doc.file then
		return false, _("No book is open.")
	end
	if not scanner then
		return false, _("Image Browser failed to load its scanner module. Try reinstalling the plugin.")
	end
	...
	if type(doc.getDocumentFileContent) ~= "function" then
		return false, _("Image Browser works with EPUB books only (this document format is not supported).")
	end
	return true
end
...
function ImageBrowser:_makeReader()
...
function ImageBrowser:_currentSpineIndex()
	local doc = self.ui.document
...
function ImageBrowser:_cachePath()
...
function ImageBrowser:_getScan(force, cache_only)
	...
	if not force then
		...
		if
			c
			and c.version == scanner.VERSION
			and cache:readSetting("mtime") == mtime
			and cache:readSetting("size") == size
		then
			self._scan = c
			return c
		end
	...
	close()
	if not ok then
		logger.warn("ImageBrowser: scan failed:", result)
...
local function _flatten_on_white(bb)
	...
	local out_type = (btype == Blitbuffer.TYPE_BBRGB32) and Blitbuffer.TYPE_BBRGB32 or Blitbuffer.TYPE_BB8
...
function ImageBrowser:_render(read_file, im)
	...
	if data then
		if im.format == "svg" or im.is_svg_doc then
			local ok, res = pcall(RenderImage.renderSVGImageDataWithCRengine, RenderImage, data, #data)
			if ok then
				bb = res
			end
		end
	...
	if not bb then
		logger.warn("ImageBrowser: could not render image", im.path)
...
function ImageBrowser:showViewer(whole_book_once)
	...
	if not scan then
		...
		if self._scan_err == "no_container" or self._scan_err == "no_opf" then
			why = _("Image Browser works with EPUB books only.")
		else
	...
	local kept_paths = {}
	for _, im in ipairs(kept_list) do
		kept_paths[im.path] = true
	end
	...
	for _, im in ipairs(scan.images) do
		local is_shown = (kept_paths[im.path] or forced[im.path]) and not hidden[im.path]
		if is_shown then
	...
	local want_ignored_primary = self._review_ignored or (self._pending_gallery ~= nil)
	...
	if #imgs == 0 then
		...
		if self:getScope() == "read_so_far" and not whole_book_once and scope_hidden > 0 then
			local msg = scope_hidden == 1 and _("No images up to here yet — 1 further in the book.")
				or T(_("No images up to here yet — %1 further in the book."), scope_hidden)
		...
		elseif #ignored_metas > 0 then
			...
			local msg = #ignored_metas == 1 and _("No images to show — 1 was filtered out as irrelevant.")
				or T(_("No images to show — %1 were filtered out as irrelevant."), #ignored_metas)
			UIManager:show(ConfirmBox:new({
				text = msg,
	...
	-- Zooming in past fit trades that for sharpness: ImageBrowserViewer lazily
	...
	local centered_popup = _popup_is_centered(_popup_layout())
	local cap_ratio = centered_popup and ImageBrowserViewer.popup_ratio or ImageBrowserViewer.panel_ratio
	local cap_w = 2 * math.floor(Screen:getWidth() * cap_ratio)
	local cap_h = 2 * math.floor(Screen:getHeight() * (centered_popup and ImageBrowserViewer.popup_ratio or 1))
	...
	local function decode(im, hires)
		...
		if bb and not hires then
			...
			if s < 1 then
				local scaled = RenderImage:scaleBlitBuffer(bb, math.floor(w * s + 0.5), math.floor(h * s + 0.5), true)
				if scaled then
					bb = scaled
				end
			end
	...
	local start = 1
	local last = self.ui.doc_settings:readSetting("imagebrowser_last")
	if last then
	...
	local effective_scope = (self:getScope() == "read_so_far" and not whole_book_once) and "read_so_far" or "whole_book"

	local viewer
	viewer = ImageBrowserViewer:new({
		image = images_list,
		image_metas = imgs,
		...
		on_image_shown = function(meta)
			self.ui.doc_settings:saveSetting("imagebrowser_last", meta.path)
		end,
		on_hide = function(meta)
			local h = self:_hiddenPaths()
			h[meta.path] = true
			self.ui.doc_settings:saveSetting("imagebrowser_hidden", h)
		end,
		...
		on_show_in_book = function(meta)
			if not meta.spine_index or not self.ui.rolling then
				return
			end
		...
		-- imagebrowser_last lands us on the same image when it's still in scope.
		on_toggle_scope = function()
			local new_scope = effective_scope == "whole_book" and "read_so_far" or "whole_book"
			...
			UIManager:show(Notification:new({
				text = new_scope == "whole_book" and _("Mode: All images") or _("Mode: Images up to here"),
			}))
		end,
		...
		on_ignore = function(meta, tab, page)
			local h = self:_hiddenPaths()
			h[meta.path] = true
			local f = self:_forcedPaths()
			f[meta.path] = nil
			self.ui.doc_settings:saveSetting("imagebrowser_hidden", h)
			self.ui.doc_settings:saveSetting("imagebrowser_forced", next(f) and f or nil)
			self.ui.doc_settings:flush()
		...
		on_unignore = function(meta, tab, page)
			...
			local h = self:_hiddenPaths()
			h[meta.path] = nil
			self.ui.doc_settings:saveSetting("imagebrowser_forced", f)
			self.ui.doc_settings:saveSetting("imagebrowser_hidden", next(h) and h or nil)
			self.ui.doc_settings:flush()
			self._pending_gallery = { tab = tab, page = page }
			if self._viewer then
				self._viewer:onClose()
	...
	viewer.onCloseWidget = function(v)
		...
		self.ui.doc_settings:saveSetting("imagebrowser_view", view)
		self._viewer = nil
		close_reader()
	...
	self.ui.doc_settings:saveSetting("imagebrowser_last", imgs[start].path)
	local view = self.ui.doc_settings:readSetting("imagebrowser_view")
	if view and view.path == imgs[start].path and type(view.scale) == "number" and view.scale ~= 0 then
	...
	if self._pending_gallery then
		...
		if n == 0 then
			tab = (tab == "ignored") and "shown" or "ignored"
		end
	...
	local open_w, open_h = viewer._panel_w + 2, Screen:getHeight()
	if viewer._centered_popup then
		open_x = math.floor((Screen:getWidth() - viewer._panel_w) / 2)
		open_y = math.floor((Screen:getHeight() - viewer._panel_h) / 2)
		open_h = viewer._panel_h
	end
	if not G_reader_settings:isTrue(SHADOW_KEY) then
		local metrics = viewer:_shadowMetrics(Screen.night_mode)
		open_w = viewer._panel_w + metrics.right_width - metrics.right_overlap + 1
		open_h = open_h + metrics.bottom_height - metrics.bottom_overlap + 1
	end
	-- Refresh isolation: ImageBrowser lives in its own refresh world. Snapshot the
	-- reader's ghost-clear counter and reset it to 0 for the session, so the
	-- reader's accumulated count can't promote a ImageBrowser refresh into a
	-- full-screen flash, and ImageBrowser's own refreshes don't push the reader
	...
	UIManager:show(
		viewer,
		Device:hasKaleidoWfm() and "partial" or "ui",
		Geom:new({
			x = open_x,
			y = open_y,
			w = math.min(Screen:getWidth() - open_x, open_w),
			h = math.min(Screen:getHeight() - open_y, open_h),
		}),
		nil,
		nil,
		true
...
-- Reads the installed version straight from _meta.lua, so the menu always
-- reflects what's actually on disk (no separate constant to fall out of sync).
...
function ImageBrowser:addToMainMenu(menu_items)
	menu_items.imagebrowser = {
		text = _("Image Browser"),
		sorting_hint = "tools",
...
-- Which gesture (if any) currently triggers ImageBrowser in this context —
...
function ImageBrowser:_gestureLabel()
	local g = self.ui and self.ui.gestures
	local found = {}
	if g and type(g.gestures) == "table" then
...
function ImageBrowser:_menuItems()
	...
	local function popup_layout_item(value, text, help)
		return {
			text = text,
			help_text = help,
			radio = true,
			checked_func = function()
				return _popup_layout() == value
			end,
			callback = function()
				_set_popup_layout(value)
			end,
		}
	end
	return {
		{
			-- which gesture opens ImageBrowser here; tap for the how-to (KOReader
			...
			-- dimmed label.
			text_func = function()
				return self:_gestureLabel()
			end,
			keep_menu_open = true,
			help_text = _(
				"Assign or change it under Taps and gestures → Gesture manager → (pick a gesture) → Reader → 'Open Image Browser'."
			),
			callback = function()
				UIManager:show(InfoMessage:new({
					text = _(
						"To open Image Browser with a single gesture:\n\nSettings → Taps and gestures → Gesture manager → pick a gesture → Reader → 'Open Image Browser'."
					),
				}))
			end,
		},
		{
			...
			help_text = _(
				"Browse the maps, family trees and other reference images found in this book, without losing your reading position. Tip: bind the gesture action 'Open Image Browser' for one-touch access."
			),
			...
			callback = function(touchmenu_instance)
				...
				UIManager:scheduleIn(0.3, function()
					...
					if self._viewer and not self:_hasGesture() and not G_reader_settings:isTrue(GESTURE_TIP_KEY) then
						G_reader_settings:saveSetting(GESTURE_TIP_KEY, true)
						UIManager:show(InfoMessage:new({
							text = _(
								"Tip: open Image Browser instantly with a gesture.\n\nSet one under Settings → Taps and gestures → Gesture manager → pick a gesture → Reader → 'Open Image Browser'.\n\n(This tip is shown only once.)"
							),
						}))
					end
				end)
		...
		},
		{
			...
			sub_item_table = {
				...
				},
				{
					...
					help_text = _(
						"While the viewer is open, a tap along the top edge of the screen opens KOReader's top menu (only the top menu, never the bottom one) over Image Browser, instead of doing nothing. Turn off to keep the top edge inert."
					),
					checked_func = function()
				...
				},
				{
					...
					help_text = _(
						"Remove the panel's drop shadow. The shadow is a dithered gradient — the main cause of e-ink ghosting behind the panel — so turn it off if a ghost lingers after closing Image Browser. No visible effect on LCD screens."
					),
					checked_func = function()
				...
				},
				{
					...
					help_text = _(
						"Image Browser caches its scan of the book. Use this if the book file was replaced or images seem out of date."
					),
					keep_menu_open = true,
					callback = function()
