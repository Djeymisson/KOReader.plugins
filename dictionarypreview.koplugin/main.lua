...
local HEADER_WORD_FACE = getUIFontFace("cfont", math_max(10, UI_FONT_SIZE - 3))
local HEADER_META_SIZE = math_max(10, UI_FONT_SIZE - 6)
...
local BUTTON_SEPARATOR_WIDTH = math_max(1, Screen:scaleBySize(1))
...
local function estimateHtmlLineCount(html, content_width, font_size)
    ...
    local average_char_width = math_max(1, font_size * 0.50)
    local chars_per_line = math_max(12, math_floor(content_width / average_char_width))
    local lines = 0
    ...
    for raw_line in text:gmatch("(.-)\n") do
        ...
        if line ~= "" then
            lines = lines + math_max(1, math_ceil(#line / chars_per_line))
        end
    end

    return math_max(1, lines)
end
...
local function getHtmlHeightProfile(html, content_width, font_size, max_html_height)
    ...
    local line_height = math_ceil(font_size * 1.30)
    local safety_lines = estimated_lines <= 2 and 0.08 or estimated_lines <= 3 and 0.18 or 0.35
    local estimated_height = math_ceil((estimated_lines + safety_lines) * line_height + Screen:scaleBySize(1))
    local base_height = math_max(Screen:scaleBySize(22), math_ceil(font_size * 1.10))
    local min_height = math_max(base_height, estimated_height)

    if max_html_height and max_html_height > 0 then
        min_height = math_min(max_html_height, min_height)
    end

    local compact_cap
    local compact_min_height
    if estimated_lines <= 3 then
        compact_cap = math_ceil((estimated_lines + 0.35) * line_height + Screen:scaleBySize(4))
        compact_min_height = math_min(max_html_height or COMPACT_MIN_HTML_HEIGHT, COMPACT_MIN_HTML_HEIGHT)
        compact_cap = math_max(compact_cap, compact_min_height)
    end
    ...
    for _, box in ipairs(boxes) do
        if type(box) == "table" and box.y and box.h then
            ...
            if box_top and box_bottom then
                top = top and math_min(top, box_top) or box_top
                bottom = bottom and math_max(bottom, box_bottom) or box_bottom
            end
        end
    end
...
function PreviewButton:init()
    ...
    local inner_w = math_max(1, outer_w - 2 * bordersize - 2 * padding_h)
    local inner_h = math_max(1, outer_h - 2 * bordersize - 2 * padding_v)
    local label
...
function DictionaryPreviewPopup:getWidgetSize(widget)
    -- Avoids allocating a wrapper closure: this runs several times per
    -- popup build (header word/meta/count widgets, buttons), including on
    -- every prev/next navigation between dictionary results.
...
function DictionaryPreviewPopup:makeHeader(content_width)
    ...
    if count_widget then
        dict_width = math_max(1, content_width - count_width - HEADER_COUNT_GAP)
    end
    ...
    local meta_height = math_max(dict_size.h or 0, count_height)
    ...
    local separator = LineWidget:new({
        ...
        dimen = Geom:new({
            w = content_width,
            h = math_max(1, Screen:scaleBySize(1)),
        }),
    })
...
function DictionaryPreviewPopup:init()
    ...
    local max_popup_height = math_floor(screen_height * PANEL_MAX_HEIGHT_RATIO)
    ...
    local content_width = math_max(MIN_CONTENT_WIDTH, self.width - CONTENT_PADDING_LEFT - CONTENT_PADDING_RIGHT)
...
function DictionaryPreviewPopup:shouldAnchorTop(card_height)
    ...
    local space_above = math_max(0, bounds.top - FLOATING_EDGE_MARGIN - FLOATING_SELECTION_GAP)
    local space_below = math_max(0, screen_height - bounds.bottom - FLOATING_EDGE_MARGIN - FLOATING_SELECTION_GAP)
...
function DictionaryPreviewPopup:makeButtons(width)
    ...
    local separator_count = math_max(0, button_count - 1)
    local available_button_width = math_max(1, width - separator_width * separator_count)
    local button_width = math_floor(available_button_width / button_count)
...
function DictionaryPreviewPopup:makeSizedHtmlWidget(content_width, max_height, height_profile)
    ...
    measure_height = math_max(1, math_min(max_height, measure_height))
    ...
    local ok, single_page_height = pcall(htmlwidget.getSinglePageHeight, htmlwidget)

    if ok and type(single_page_height) == "number" and single_page_height > 0 then
        local measurement_safety = is_compact and COMPACT_HEIGHT_SAFETY or math_ceil(self.doc_font_size * 0.35)
        height = math_ceil(single_page_height + measurement_safety)
        height = math_max(min_height, math_min(max_height, height))

        if is_compact and compact_min_height then
            height = math_max(compact_min_height, height)
        end
    else
        height = math_max(1, math_min(max_height, height))
...
function DictionaryPreview:beginNativeDictionaryPopup(dict_close_callback)
    ...
    return function(...)
        if not closed then
            closed = true
            plugin.native_dict_popup_count = math_max(0, (plugin.native_dict_popup_count or 1) - 1)
...
-- Shared by every entry point that hands a highlight back to the reader:
-- a hold-position selection only becomes usable once this runs.
local function ensureHighlightFromHoldPos(highlight)
    if not highlight.selected_text and highlight.hold_pos and type(highlight.highlightFromHoldPos) == "function" then
        pcall(highlight.highlightFromHoldPos, highlight)
    end
end
...
function DictionaryPreview:rememberSelection(dict_self)
    ...
    ensureHighlightFromHoldPos(highlight)
