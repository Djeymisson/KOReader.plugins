...
local PAGE_DOT_FACE = Font:getFace(
	"cfont",
	math.max(14, (HEADER_BUTTON_FONT_SIZE or UI_FONT_SIZE or 20) - 5)
)
...
local TAB_LABEL_FACE = Font:getFace(
	"cfont",
	math.max(13, (HEADER_MENU_FONT_SIZE or UI_FONT_SIZE or 20) - 1)
)
...
local function makeSideTab(text, width, height, active)
	local label = makeTextLabel(
		text,
		HEADER_MENU_FACE,
		math.max(1, width - 2 * SIDE_TAB_PADDING_H),
		active
	)
...
function HeaderPageButton:init()
	...
	local label = makeTextLabel(
		self.text,
		self.face or HEADER_BUTTON_FACE,
		label_width,
		self.bold ~= false
	)
...
function CardTabButton:paintTo(bb, x, y)
	...
	if self.bottom_cover then
		...
		self.bottom_cover:paintTo(bb, x + SIDE_TAB_BORDER_SIZE, y + top_offset + frame_height - SIDE_TAB_BORDER_SIZE)
	end
...
function CarouselRow:buildSideTabs()
	...
	for page_index = PAGE_DICTIONARY, PAGE_WIKIPEDIA do
		...
		local width = page_index == PAGE_WIKIPEDIA
			and math.max(1, self.card_width - base_width * 2)
			or base_width
		local x = active_x + offset * base_width

		self.side_tab_widgets[page_index] = makeSideTab(
			PAGE_TITLES[page_index],
			width,
			height,
			page_index == self.active_index
		)
...
function LookupPreviewPopup:makeHeader(payload, content_width)
	...
	local text_width = menu_button
		and math.max(1, content_width - menu_width - HEADER_TITLE_MENU_GAP)
		or content_width
	local text_items = {
...
function LookupPreviewPopup:useTabsMode()
	return self.plugin
		and type(self.plugin.useTabsMode) == "function"
		and self.plugin:useTabsMode()
end
...
function LookupPreviewPopup:makeCardTabs(content_width, active_index)
	...
	if used_width > content_width then
		...
		for index, spec in ipairs(specs) do
			spec.width = index == #specs
				and math.max(1, content_width - base_width * (#specs - 1))
				or base_width
...
function LookupPreviewPopup:makeCard(payload, card_width, card_height)
	...
	local html_height, buttons_gap = self:getCardDimensions(
		card_width,
		card_height,
		header,
		buttons,
...
function LookupPreviewPopup:getPageMenuAnchor()
	...
	return Geom:new({
		...
		y = ((self.visible_dimen and self.visible_dimen.y) or CARD_EDGE_MARGIN) + CARD_BORDER_SIZE + CARD_PADDING_TOP,
		w = HEADER_MENU_WIDTH,
...
function LookupPreviewPopup:updateCurrentPage(index)
	...
	if self.card_container and self.card_container.replaceActiveCard then
		...
		self:markDirty("partial", self:getActiveCardDirtyDimen())
		return true
	end
